...
def create_app(test_config=None):
    ...
    def index():
        lab_count = db.session.scalar(select(func.count()).select_from(Lab))
        return render_template("index.html", lab_count=lab_count)
    ...
    def reservation_form():
        ...
        labs = db.session.scalars(select(Lab).where(Lab.status == "可预约").order_by(Lab.id)).all()
        return render_template("reservation_form.html", labs=labs, selected_lab_id=selected_lab_id)
