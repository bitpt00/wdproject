# 实验4　D08—D10：从开放时段到 V0.4 学生预约闭环

| 项目 | 内容 |
|---|---|
| 建议用时 | 100—120 分钟 |
| 开发起点 | `dev-v0.3` 角色与权限 |
| 开发终点 | `dev-v0.4` 学生预约闭环 |
| 验收证据 | 可预约时段、预约记录、取消与释放、18 项自动测试 |

## 4.1 实验目标与成果

本实验第一次完成系统的核心业务。用户不再填写一个与时间无关的静态表单，而是先选择一间实验室的真实开放时段，再提交、查看和取消自己的预约。
...
本实验是 D01—D16 功能推进过程中操作量较大的一段。每一步都要实际运行，不能一次性覆盖到 D10。
...
- 数据已经混乱：停止程序，把 `instance` 重命名为 `instance-old-D10`，再由 D10 重建。确认新数据库和测试均正确后，再决定是否保留旧数据备份。
...
1. 实验室是资源，时段是可以实际分配的资源单位。
2. 表单不重复收集系统已经确定的实验室、日期和时间，可以减少输入错误。
3. `Booking` 通过外键连接学生与时段，并通过状态记录业务过程。
4. 服务器端校验决定数据是否写入数据库，页面输入限制只能作为辅助。
5. 取消采用状态变化而不是删除，可以为后续审批、追踪和统计保留依据。

思考：预约取消后为什么要保留记录，同时重新释放原时段？
