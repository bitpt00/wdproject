...
| 开发起点 | 空的个人项目目录 |
| 开发终点 | `dev-v0.1` 静态原型 |
| 验收证据 | 3 个页面、指定页面文字与截图、6 项自动测试 |

## 1.1 实验目标与成果

本实验从一个空项目文件夹开始，依次得到四种清楚可见的状态：
...
本实验不要求自行设计代码。每次只从课程资料中复制指定的完整文件，再启动、观察并解释结果。不要一次性复制到 D03；每完成一步都应启动一次，亲眼看到系统发生变化。

D03 结束时，系统只有页面和示例数据，预约表单还不能保存。这是有意设计的版本边界，不是程序故障。

## 1.2 实验准备与文件约定

为了避免“文件到底放在哪里”的问题，本课程统一使用下面两个位置：

- `<课程资料>`：教师发放或从仓库下载到 `D:\campus-lab-course` 的课程资源文件夹。
- `D:\campus-lab-work`：每位学生自己的项目文件夹。
...
- 起点：电脑已安装 Python 3.11，尚无个人项目文件夹。
- 终点：`D:\campus-lab-work` 中有项目专用虚拟环境 `.venv`，Flask 和 pytest 已安装。
...
应理解：Python 是运行程序的基础；虚拟环境把本项目的依赖与电脑中的其他 Python 项目隔离；`requirements.txt` 让不同电脑安装相同版本的软件包。
...
把下列三个文件复制到 `D:\campus-lab-work`：

| 操作 | 完整源文件 | 目标文件 |
|---|---|---|
| 新增 | [`.gitignore`](../../教学资源/逐步开发代码/D00_S0起点/.gitignore) | `D:\campus-lab-work\.gitignore` |
| 新增 | [`requirements.txt`](../../教学资源/逐步开发代码/D00_S0起点/requirements.txt) | `D:\campus-lab-work\requirements.txt` |
| 新增 | [`初始化开发环境.bat`](../../教学资源/逐步开发代码/D00_S0起点/初始化开发环境.bat) | `D:\campus-lab-work\初始化开发环境.bat` |

#### 操作 D00-3　初始化项目专用环境

1. 在 `D:\campus-lab-work` 中双击 `初始化开发环境.bat`。
2. 保持网络连接，等待脚本创建环境并安装依赖。
3. 看到 `Development environment is ready.` 后按任意键关闭窗口。
4. 回到文件资源管理器，确认项目中新增 `.venv` 文件夹。

#### 操作 D00-4　用三条命令核对环境

1. 在 `D:\campus-lab-work` 文件夹空白处单击鼠标右键，选择“在终端中打开”。
...
- 看不到 `.venv`：确认脚本确实位于 `D:\campus-lab-work`，不要在压缩包预览窗口中运行。
- 路径出现异常：确认项目路径是英文且没有空格，即 `D:\campus-lab-work`。

仍无法解决时，关闭所有相关终端，把个人项目中创建失败的 `.venv` 重命名为 `.venv-old`，重新复制 D00 的三个文件并再次初始化。新环境验收通过后再删除 `.venv-old`；不要改动课程资料中的 D00 完整恢复包。

D00 完成标志：项目专用 Python 环境已经存在；依赖版本已固定；此时没有网站是正确结果。

---

## 1.4 D01：第一个 Flask 页面
...
| 操作 | 完整源文件 | 目标文件 |
|---|---|---|
| 新增 | [`app.py`](../../教学资源/逐步开发代码/D01_第一个Flask页面/app.py) | `D:\campus-lab-work\app.py` |
| 新增 | [`VERSION`](../../教学资源/逐步开发代码/D01_第一个Flask页面/VERSION) | `D:\campus-lab-work\VERSION` |
| 新增 | [`启动dev-v0.1.bat`](../../教学资源/逐步开发代码/D01_第一个Flask页面/启动dev-v0.1.bat) | `D:\campus-lab-work\启动dev-v0.1.bat` |

不要复制源文件夹中的 `.venv`；课程资料中本来就不提供 `.venv`。

#### 操作 D01-2　认识 app.py 的四个组成部分

打开目标文件 `D:\campus-lab-work\app.py`。此时只需要找到下面四个结构，不要求背诵语法：
...
1. 双击 `D:\campus-lab-work\启动dev-v0.1.bat`。
...
| 操作 | 完整源文件 | 目标文件 |
|---|---|---|
| 整文件替换 | [`app.py`](../../教学资源/逐步开发代码/D02_Jinja首页与样式/app.py) | `D:\campus-lab-work\app.py` |
| 整文件替换 | [`VERSION`](../../教学资源/逐步开发代码/D02_Jinja首页与样式/VERSION) | `D:\campus-lab-work\VERSION` |
...
1. 在 `D:\campus-lab-work` 新建文件夹 `templates`。
2. 在 `D:\campus-lab-work` 新建文件夹 `static`。
...
| 操作 | 完整源文件 | 目标文件 |
|---|---|---|
| 新增 | [`templates/base.html`](../../教学资源/逐步开发代码/D02_Jinja首页与样式/templates/base.html) | `D:\campus-lab-work\templates\base.html` |
| 新增 | [`templates/index.html`](../../教学资源/逐步开发代码/D02_Jinja首页与样式/templates/index.html) | `D:\campus-lab-work\templates\index.html` |
| 新增 | [`static/style.css`](../../教学资源/逐步开发代码/D02_Jinja首页与样式/static/style.css) | `D:\campus-lab-work\static\style.css` |
...
*图 1-3　D02 使用 Jinja2 与 CSS 的首页*

这次变化不是增加业务功能，而是建立后续页面共同使用的组织方式。可以做一个可逆的小观察：暂时修改 `<h1>` 中的一个字，保存并刷新；观察变化后立即恢复源文件，由此区分模板负责内容、CSS 负责外观。

### 1.5.3 故障排查与完成检查

- 出现 `TemplateNotFound: index.html`：检查文件是否准确位于 `D:\campus-lab-work\templates\index.html`。
...
| 操作 | 完整源文件 | 目标文件 |
|---|---|---|
| 整文件替换 | [`app.py`](../../教学资源/逐步开发代码/D03_V0.1静态原型/app.py) | `D:\campus-lab-work\app.py` |
| 整文件替换 | [`VERSION`](../../教学资源/逐步开发代码/D03_V0.1静态原型/VERSION) | `D:\campus-lab-work\VERSION` |
| 整文件替换 | [`templates/base.html`](../../教学资源/逐步开发代码/D03_V0.1静态原型/templates/base.html) | `D:\campus-lab-work\templates\base.html` |
| 整文件替换 | [`templates/index.html`](../../教学资源/逐步开发代码/D03_V0.1静态原型/templates/index.html) | `D:\campus-lab-work\templates\index.html` |
...
| 操作 | 完整源文件 | 目标文件 |
|---|---|---|
| 新增 | [`templates/labs.html`](../../教学资源/逐步开发代码/D03_V0.1静态原型/templates/labs.html) | `D:\campus-lab-work\templates\labs.html` |
| 新增 | [`templates/reservation_form.html`](../../教学资源/逐步开发代码/D03_V0.1静态原型/templates/reservation_form.html) | `D:\campus-lab-work\templates\reservation_form.html` |

`labs.html` 使用 Jinja2 的 `for` 循环把三条数据重复生成三张卡片；预约表单中的输入框此时用于确认需求字段，提交按钮故意处于不可用状态。

#### 操作 D03-4　新增自动测试

1. 在 `D:\campus-lab-work` 新建文件夹 `tests`。
2. 复制下面两个文件。

| 操作 | 完整源文件 | 目标文件 |
|---|---|---|
| 新增 | [`tests/test_app.py`](../../教学资源/逐步开发代码/D03_V0.1静态原型/tests/test_app.py) | `D:\campus-lab-work\tests\test_app.py` |
| 新增 | [`运行测试.bat`](../../教学资源/逐步开发代码/D03_V0.1静态原型/运行测试.bat) | `D:\campus-lab-work\运行测试.bat` |
...
2. 双击 `D:\campus-lab-work\运行测试.bat`。
3. 等待测试结束。
4. 正确结果必须包含：
...
1. D00：可重复的环境是软件能够在不同电脑上运行的前提。
2. D01：Web 系统的最小过程是“请求—路由—响应”。
3. D02：程序逻辑、页面结构和页面样式应分开管理。
4. D03：可以先用静态原型确认界面和功能范围，再接入数据库与真实业务。

思考：为什么每完成一次整文件替换都要立即运行和核对，而不能一次复制完 D01—D03 后只检查最终页面？
